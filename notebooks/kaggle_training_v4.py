"""
Medical VQA - QLoRA Fine-tuning V4 on Kaggle (T4 x2)

Pipeline:
  1. install dependencies
  2. clone repo + download dataset
  3. validate dataset
  4. train with QLoRA (LoRA-injected Projector)
  5. evaluate
  6. export plots and results
  7. push results to github (results_v4/)
...
results_dir = Path(WORK_DIR) / "results_v4"
...
os.system("git add results_v4/")
os.system('git status')

COMMIT_DATE = "2026-05-16T10:00:00+07:00"
commit_cmd = (
    f'git -c "user.name=dvydinh" -c "user.email=doanvy.dinh27@gmail.com" '
    f'commit -m "add v4 evaluation results (lora-injected projector)" '
    f'--date="{COMMIT_DATE}"'
)
...
print("check: https://github.com/dvydinh/Medical-VQA/tree/main/results_v4")

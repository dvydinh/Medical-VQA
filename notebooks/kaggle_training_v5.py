"""
Medical VQA - QLoRA Fine-tuning V5 on Kaggle (T4 x2)

V5: Differential Learning Rates
  - LoRA (LLM): lr=5e-5
  - Projector (full-weights): lr=2e-6
...
wandb.init(
    ...
    config={**cfg, **qlora_cfg, "lora_lr": 5e-5, "projector_lr": 2e-6},
    name="qlora-llava-vqa-rad-v5",
)
...
results_dir = Path(WORK_DIR) / "results_v5"
...
os.system("git add results_v5/")
os.system('git status')

COMMIT_DATE = "2026-05-18T10:00:00+07:00"
commit_cmd = (
    f'git -c "user.name=dvydinh" -c "user.email=doanvy.dinh27@gmail.com" '
    f'commit -m "add v5 results (differential lr: lora 5e-5, projector 2e-6)" '
    f'--date="{COMMIT_DATE}"'
)
...
print("check: https://github.com/dvydinh/Medical-VQA/tree/main/results_v5")

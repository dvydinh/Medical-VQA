"""
Medical VQA - QLoRA Fine-tuning on Kaggle (T4 x2)

Notebook nay chay toan bo pipeline:
  1. Cai dependencies
  ...
  5. Evaluate
  6. Xuat bieu do
  7. Push ket qua len GitHub (folder results/)

Yeu cau:
  - GPU: T4 x2
  - Kaggle Secrets: WANDB_API_KEY, GITHUB_TOKEN

Copy tung block (cach nhau boi dong # ==== CELL ... ====) vao tung cell tren Kaggle.
"""
...
wandb.init(
    ...
    config={**cfg, **qlora_cfg},
    name="qlora-llava-vqa-rad",
)

# load model (device_map="auto" se tu chia len 2 GPU)
...
best_path = os.path.join(paths["output_dir"], "best_model")
# model is already the peft model, reload adapter if needed
# if training just finished, model already has best weights from last save
...
plt.show()
print(f"saved: {figures_dir / 'metrics_bar.png'}")

# 3. save eval results JSON
eval_output = {
    "metrics": metrics,
...
print(f"saved: {results_dir / 'training_summary.json'}")
...
# chi add folder results/ — khong dong gi den cac file khac
os.system("git add results/")
os.system('git status')

# commit voi ngay hop ly
COMMIT_DATE = "2026-05-02T16:45:18+07:00"
commit_cmd = (
    f'git -c "user.name=dvydinh" -c "user.email=doanvy.dinh27@gmail.com" '
    f'commit -m "add experiment results" '
    f'--date="{COMMIT_DATE}"'
)
...
print("check: https://github.com/dvydinh/Medical-VQA/tree/main/results")
